...
import com.taotao.boot.security.spring.event.domain.TtcUserStatus;
...
public class OAuth2AccountStatusManager {
    ...
    public void lock(String username) {
        ...
        if (ObjectUtils.isNotEmpty(userId)) {
            accountStatusChanger.process(new TtcUserStatus(userId, DataItemStatusEnum.LOCKING.name()));
    ...
    public void enable(String userId) {
        if (ObjectUtils.isNotEmpty(userId)) {
            accountStatusChanger.process(new TtcUserStatus(userId, DataItemStatusEnum.ENABLE.name()));
        }
    }
