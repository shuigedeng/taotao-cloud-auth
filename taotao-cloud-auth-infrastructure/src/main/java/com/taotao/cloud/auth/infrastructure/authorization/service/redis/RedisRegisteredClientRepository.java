...
public class RedisRegisteredClientRepository extends JpaRegisteredClientRepository {
	...
	public RegisteredClient findById(String id) {
		RegisteredClient registeredClientByRedis =
			(RegisteredClient) redisRepository.opsForValue()
				.get(PREFIX + REGISTERED_CLIENT_ID + id);
	...
	public RegisteredClient findByClientId(String clientId) {
		...
		if (registeredClientByRedis == null) {
			...
			log.debug("根据 clientId：{} 直接查询数据库中的客户：{}", clientId,
				registeredClientByDatabase);
			...
			log.debug("根据 clientId：{} 直接查询Redis中的客户：{}", clientId,
				registeredClientByRedis);
			registeredClientResult = registeredClientByRedis;
		}
		return registeredClientResult;
	}
	...
	public void set(RegisteredClient registeredClient, long timeout, TimeUnit unit) {
		redisRepository
			.opsForValue()
			.set(PREFIX + REGISTERED_CLIENT_ID + registeredClient.getId(), registeredClient,
				timeout, unit);
		redisRepository
			.opsForValue()
			.set(PREFIX + REGISTERED_CLIENT_CLIENT_ID + registeredClient.getClientId(),
				registeredClient,
				timeout,
				unit);
	}
