...
public final class RedisOAuth2AuthorizationService extends JpaOAuth2AuthorizationService {
	...
	public void remove(OAuth2Authorization authorization) {
		if (authorization != null) {
			...
			if (accessToken != null) {
				...
				redisRepository.del(PREFIX + OAUTH2_AUTHORIZATION_TOKEN_TYPE
	...
	public OAuth2Authorization findById(String id) {
		...
		OAuth2Authorization oauth2AuthorizationRedis =
			(OAuth2Authorization) redisRepository.opsForValue()
				.get(PREFIX + OAUTH2_AUTHORIZATION_ID + id);
		...
		if (oauth2AuthorizationRedis == null) {
			...
			LogUtils.debug("根据 id：{} 直接查询数据库中的授权：{}", id,
	...
	public OAuth2Authorization findByToken(String token, OAuth2TokenType tokenType) {
		...
		OAuth2Authorization oauth2AuthorizationByRedis = (OAuth2Authorization) redisRepository
			.opsForValue()
			.get(PREFIX + OAUTH2_AUTHORIZATION_TOKEN_TYPE + tokenTypeValue + ":" + token);
		...
		if (oauth2AuthorizationByRedis == null) {
			...
			LogUtils.debug("根据 token：{}、tokenType：{} 直接查询数据库中的客户：{}", token, tokenType,
				oauth2AuthorizationByDatabase);
			...
			LogUtils.debug("根据 token：{}、tokenType：{} 直接查询Redis中的客户：{}", token, tokenType,
	...
	public void set(OAuth2Authorization authorization, long timeout, TimeUnit unit) {
		redisRepository
			.opsForValue()
			.set(PREFIX + OAUTH2_AUTHORIZATION_ID + authorization.getId(), authorization, timeout,
				unit);
		...
		if (accessToken != null) {
			...
			if (token != null) {
				...
				redisRepository
					.opsForValue()
					.set(
						PREFIX + OAUTH2_AUTHORIZATION_TOKEN_TYPE
							+ OAuth2TokenType.ACCESS_TOKEN.getValue() + ":"
							+ tokenValue,
						authorization,
						timeout,
